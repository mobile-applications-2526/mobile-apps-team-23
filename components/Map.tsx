  ...
  return (
    <View style={{ flex: 1 }}>
      <MapView
        ...
        onRegionChangeComplete={(region) => setIsZoomedIn(region.latitudeDelta <= 0.05)}
      >
        {locations
              ...
              >
                ...
                >
                  <View style={styles.callout}>
                    <Text style={styles.calloutName}>{loc.userinfo?.name ?? "Onbekend"}</Text>

                    {latestPost ? (
                      <>
                        {latestPost.image_url && (
                          <Image source={{ uri: latestPost.image_url }} style={styles.calloutImage} />
                        )}
                        {latestPost.title && <Text style={styles.calloutTitle}>{latestPost.title}</Text>}
                        {latestPost.description && (
                          <Text style={styles.calloutDescription}>{latestPost.description}</Text>
                        )}
                      </>
                    ) : (
                      <Text style={styles.calloutDescription}>Geen recente post</Text>
                    )}
                  </View>
                </Callout>
              </Marker>
      ...
      <View style={styles.fabContainer}>
        <TouchableOpacity style={styles.fabButton} onPress={goToUserLocation} activeOpacity={0.8}>
